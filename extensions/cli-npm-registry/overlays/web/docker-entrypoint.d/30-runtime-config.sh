...
envsubst '${SKILLHUB_WEB_API_BASE_URL} ${SKILLHUB_PUBLIC_BASE_URL} ${SKILLHUB_CLI_NPM_REGISTRY} ${SKILLHUB_WEB_AUTH_DIRECT_ENABLED} ${SKILLHUB_WEB_AUTH_DIRECT_PROVIDER} ${SKILLHUB_WEB_AUTH_SESSION_BOOTSTRAP_ENABLED} ${SKILLHUB_WEB_AUTH_SESSION_BOOTSTRAP_PROVIDER} ${SKILLHUB_WEB_AUTH_SESSION_BOOTSTRAP_AUTO}' \
...
printf '%s\n' \
  ...
  > "$guide_url_config"
# The guide is served as a static file, so substitute the CLI npm registry at
# startup. Without this the literal `${SKILLHUB_CLI_NPM_REGISTRY}` placeholder
# would be served to agents.
envsubst '${SKILLHUB_CLI_NPM_REGISTRY}' \
