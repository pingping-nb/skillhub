...
Ask the user which scope to install to (`user` or `project`) instead of assuming `user`. Omit `--version` only when the user did not select one. Never add `--force` unless the user approves replacing a verified same-source installation.

If `install` fails with `partial-target install would create inconsistent versions`, the Skill is already recorded in another scope (often a stale entry from an interrupted install). Do not retry `install` blindly. Instead, either upgrade the existing installation with `skillhub upgrade @<namespace>/<slug>`, or clear the stale entry with `skillhub remove @<namespace>/<slug> --all --registry https://skillhub.your-company.com` before reinstalling.
...
Report the installed coordinate and version, registry source, Agent and installation directory, and whether `SKILL.md` and `.skillhub/metadata.json` exist. Do not claim success if installation, destination loading, or integrity verification failed.
