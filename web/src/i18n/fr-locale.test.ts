...
describe('french locale', () => {
  it('mirrors the english key tree', () => {
    expect(normalizedLeafKeys(fr).sort()).toEqual(normalizedLeafKeys(en).sort())
  })
  ...
  it('translates core navigation labels', () => {
    ...
    expect(fr.nav.home.length).toBeGreaterThan(0)
    expect(fr.login.title.length).toBeGreaterThan(0)
  })
})
