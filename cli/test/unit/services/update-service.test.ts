...
describe('UpdateService', () => {
  test('npx install mode only suggests next command', async () => {
    ...
    expect(result.next).toContain('npx @neobards/skillhub@latest')
  })
  ...
  test('npm-global install mode runs npm update', async () => {
    ...
    expect(executed).toEqual(['npm', 'install', '-g', '@neobards/skillhub@latest'])
  })
  ...
  test('bun-global install mode runs bun update', async () => {
    ...
    expect(result.updated).toBe(true)
    expect(executed).toEqual(['bun', 'add', '-g', '@neobards/skillhub@latest'])
  })
