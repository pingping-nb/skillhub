...
| Deploy     | Helm chart (`charts/skillhub/`), K8s manifests, Docker     |
| Build      | `make dev-all` (dev), `make staging` (pre-PR)              |
| Docs       | `docs/` (design), `docs/skillhub/` (published user guide)  |
...
│
├── builtin-skills/                  # Official starter skill pool
│   ├── catalog.json                 # Candidate catalog (source of truth)
│   └── skills/                      # Skill packages (built by scripts/build-builtin-skills.py)
│
├── charts/skillhub/                 # Helm chart (values.schema.json + unit tests in tests/)
├── examples/python/                 # Python API client example (skillhub_client.py)
│
├── docs/                            # Design documents (source of truth)
│   ├── prds/                        # Product requirement documents
│   ├── skillhub/                    # VitePress user guide (DEPLOYED to iflytek.github.io/skillhub)
│   └── superpowers/                 # Internal tooling docs
│
├── document/                        # Legacy Docusaurus site (hand-written, NOT deployed by CI)
│   ├── docs/                        # Markdown documentation (01-getting-started ... 05-reference)
│   └── i18n/                        # Internationalization
│
├── weekly/                          # Weekly community reports (built into docs site by CI)
...
| Add CLI command | `cli/src/commands/` (Bun; run `make test-cli` / `make typecheck-cli`) |
...
- `cli/src/generated/` — generated by `bun run scripts/generate-pkg-info.ts` (runs automatically via prebuild/pretest)
- `document/docs/` — legacy Docusaurus mirror; the deployed user guide lives in `docs/skillhub/` (VitePress).
  Confirm with the maintainer which site a docs change should target before editing either one.
...
make test-cli                 # CLI unit tests (bun test, in cli/)
make typecheck-cli            # CLI type check
./scripts/smoke-test.sh       # API smoke test
make namespace-smoke          # Namespace workflow smoke test
make test-builtin-skills      # Builtin skills manifest/packaging tests
make build-builtin-skills     # Rebuild builtin skill packages (python3)
```

### Documentation Sites

```bash
make docs-dev      # VitePress dev server (docs/skillhub, npm)
make docs-build    # Build the published user guide
```

- **`docs/skillhub/`** (VitePress) is the deployed user guide at `iflytek.github.io/skillhub`
  (built by `.github/workflows/deploy-docs.yml`, which also builds `weekly/` reports into it).
- **`document/`** is a legacy hand-written Docusaurus site, not deployed by CI. Do not assume
  the two are in sync; ask the maintainer which one a docs PR should target.
...
- [ ] If `cli/` changed: `make test-cli` and `make typecheck-cli` pass
...
- **`cli/package.json`** — CLI dependencies and Bun scripts
...
- **`cli/src/`** — CLI commands and services (Bun + TypeScript)
- **`charts/skillhub/`** — Helm chart (templates, values.schema.json, unit tests)
